...
import { ProviderSettings, isRouterName, RouterModels } from "Autogenlabs/shared/api"
...
export function validateApiConfiguration(apiConfiguration: ProviderSettings): string | undefined {
	switch (apiConfiguration.apiProvider) {
		...
		case "anthropic":
			...
			break
		case "autogenlabs":
			if (!apiConfiguration.autogenLabsApiKey) {
				return i18next.t("settings:validation.apiKey")
			}
			if (!apiConfiguration.autogenLabsBaseUrl) {
