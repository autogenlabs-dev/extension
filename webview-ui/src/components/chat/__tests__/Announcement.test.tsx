...
import { Package } from "Autogenlabs/schemas"
	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 Autogenlabs Code ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `Autogenlabs Code ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 Autogenlabs Code ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`Autogenlabs Code ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
