...
export class ConversationTelemetryService {
	...
	private apiEndpoint: string = "https://api.AutoGen.bot/v1/traces"
	...
	public isOptedInToConversationTelemetry(): boolean {
		...
		const isConversationTelemetryEnabled =
			vscode.workspace.getConfiguration("AutoGen").get<boolean>("conversationTelemetry") ?? false
	...
	private async initializeTracer() {
		try {
			...
			const resource = new Resource({
				[ATTR_SERVICE_NAME]: "AutoGen-extension",
			...
			if (autogenApiKey) {
				headers["Authorization"] = `Bearer ${autogenApiKey}`
			}
			...
			// Get a tracer
			this.tracer = trace.getTracer("AutoGen-conversation-tracer")
