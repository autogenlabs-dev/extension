...
import { AutoGenIgnoreController } from "../../core/ignore/AutoGenIgnoreController"
...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = autogenIgnoreController ? autogenIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, autogenIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (autogenIgnoreController && !autogenIgnoreController.validateAccess(filePath)) {
		return null
	}
