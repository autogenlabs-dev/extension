...
class CheckpointTracker {
	...
	private constructor(provider: AutoGenProvider, taskId: string, cwd: string) {
		this.providerRef = new WeakRef(provider)
		this.taskId = taskId
		this.cwd = cwd
	}

	public static async create(taskId: string, provider?: AutoGenProvider): Promise<CheckpointTracker | undefined> {
		try {
			if (!provider) {
			...
			const enableCheckpoints = vscode.workspace.getConfiguration("AutoGen").get<boolean>("enableCheckpoints") ?? true
	...
	private static async getWorkingDirectory(): Promise<string> {
		...
		if (!cwd) {
			throw new Error("No workspace detected. Please open AutoGen in a workspace to use checkpoints.")
		}
	...
	public static async doesShadowGitExist(taskId: string, provider?: AutoGenProvider): Promise<boolean> {
	...
	public async initShadowGit(): Promise<string> {
		...
		if (await fileExistsAtPath(gitPath)) {
			...
			await git.addConfig("user.name", "AutoGen Checkpoint")
