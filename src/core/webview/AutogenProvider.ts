...
import { AutoGenCheckpointRestore, WebviewMessage } from "../../shared/WebviewMessage"
...
import { AutoGenAccountService } from "../../services/account/AutoGenAccountService"
...
export class AutoGenProvider implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "claude-dev.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "claude-dev.TabPanelProvider"
	private static activeInstances: Set<AutoGenProvider> = new Set()
	private disposables: vscode.Disposable[] = []
	private view?: vscode.WebviewView | vscode.WebviewPanel
	private AutoGen?: AutoGen
	workspaceTracker?: WorkspaceTracker
	mcpHub?: McpHub
	accountService?: AutoGenAccountService
	...
	constructor(
		...
		this.outputChannel.appendLine("AutoGenProvider instantiated")
		AutoGenProvider.activeInstances.add(this)
		this.workspaceTracker = new WorkspaceTracker(this)
		this.mcpHub = new McpHub(this)
		this.accountService = new AutoGenAccountService(this)
	...
	*/
	async dispose() {
		this.outputChannel.appendLine("Disposing AutoGenProvider...")
		...
		AutoGenProvider.activeInstances.delete(this)
	}

	// Auth methods
	async handleSignOut() {
		try {
			await this.storeSecret("autogenApiKey", undefined)
			await this.updateGlobalState("apiProvider", "openrouter")
			await this.postStateToWebview()
			vscode.window.showInformationMessage("Successfully logged out of AutoGen")
		} catch (error) {
	...
	public static getVisibleInstance(): AutoGenProvider | undefined {
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		vscode.workspace.onDidChangeConfiguration(
			async (e) => {
				...
				if (e && e.affectsConfiguration("AutoGen.mcpMarketplace.enabled")) {
	...
	async initAutoGenWithTask(task?: string, images?: string[]) {
	...
	async initAutoGenWithHistoryItem(historyItem: HistoryItem) {
		await this.clearTask()
		const { apiConfiguration, customInstructions, autoApprovalSettings, browserSettings, chatSettings } =
			await this.getState()
		this.AutoGen = new AutoGen(
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		try {
			...
			vscode.window.showErrorMessage(
				"AutoGen: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
			)
			...
			async (message: WebviewMessage) => {
				switch (message.type) {
					...
					case "newTask":
						...
						// initializing new instance of AutoGen will make sure that any agentically running promises in old instance don't affect our new task. this essentially creates a fresh slate for the new task
						await this.initAutoGenWithTask(message.text, message.images)
						break
					...
					case "autoApprovalSettings":
						if (message.autoApprovalSettings) {
							...
							if (this.AutoGen) {
								this.AutoGen.autoApprovalSettings = message.autoApprovalSettings
							}
							await this.postStateToWebview()
						}
						break
					case "browserSettings":
						if (message.browserSettings) {
							...
							if (this.AutoGen) {
								this.AutoGen.updateBrowserSettings(message.browserSettings)
							}
							await this.postStateToWebview()
						}
						break
					...
					// 	if (this.AutoGen) {
					// 		this.AutoGen.browserSession.relaunchChromeDebugMode()
					// 	}
					// 	break
					case "askResponse":
					...
					case "exportCurrentTask":
						const currentTaskId = this.AutoGen?.taskId
						if (currentTaskId) {
					...
					case "checkpointDiff": {
						if (message.number) {
							await this.AutoGen?.presentMultifileDiff(message.number, false)
						}
						break
					}
					case "checkpointRestore": {
						...
						// cancel task waits for any open editor to be reverted and starts a new AutoGen instance
						if (message.number) {
							// wait for messages to be loaded
							await pWaitFor(() => this.AutoGen?.isInitialized === true, {
								timeout: 3_000,
							}).catch(() => {
								console.error("Failed to init new AutoGen instance")
							})
							// NOTE: cancelTask awaits abortTask, which awaits diffViewProvider.revertChanges, which reverts any edited files, allowing us to reset to a checkpoint rather than running into a state where the revertChanges function is called alongside or after the checkpoint reset
							await this.AutoGen?.restoreCheckpoint(message.number, message.text! as AutoGenCheckpointRestore)
						}
						break
					}
					case "taskCompletionViewChanges": {
						if (message.number) {
							await this.AutoGen?.presentMultifileDiff(message.number, true)
						}
						break
					}
					case "cancelTask":
					...
					case "downloadMcp": {
						if (message.mcpId) {
							...
							const mcpConfig = vscode.workspace.getConfiguration("AutoGen.mcp")
					...
					// 	if (message.text) {
					// 		const response = await fetch(`https://api.AutoGen.bot/v1/mcp/marketplace/item?mcpId=${message.mcpId}`)
...
Please generate the component. After generation, verify the component code, ensure all necessary dependencies are listed or installed, and confirm everything is set up correctly.`;
	...
	async togglePlanActModeWithChatSettings(chatSettings: ChatSettings, chatContent?: ChatContent) {
		...
		if (shouldSwitchModel) {
			...
			if (newApiProvider || newModelId || newThinkingBudgetTokens !== undefined) {
				...
				if (this.AutoGen) {
					...
					this.AutoGen.api = buildApiHandler(updatedApiConfiguration)
				}
			}
		}
		...
		if (this.AutoGen) {
			this.AutoGen.updateChatSettings(chatSettings)
			if (this.AutoGen.isAwaitingPlanResponse && didSwitchToActMode) {
				this.AutoGen.didRespondToPlanAskBySwitchingMode = true
	...
	async cancelTask() {
		if (this.AutoGen) {
			const { historyItem } = await this.getTaskWithId(this.AutoGen.taskId)
			try {
				await this.AutoGen.abortTask()
			} catch (error) {
				console.error("Failed to abort task", error)
			}
			await pWaitFor(
				() =>
					this.AutoGen === undefined ||
					this.AutoGen.isStreaming === false ||
					this.AutoGen.didFinishAbortingStream ||
					this.AutoGen.isWaitingForFirstChunk, // if only first chunk is processed, then there's no need to wait for graceful abort (closes edits, browser, etc)
				{
					timeout: 3_000,
				},
			).catch(() => {
			...
			if (this.AutoGen) {
				// 'abandoned' will prevent this AutoGen instance from affecting future AutoGen instance gui. this may happen if its hanging on a streaming request
				this.AutoGen.abandoned = true
			}
			await this.initAutoGenWithHistoryItem(historyItem) // clears task again, so we need to abortTask manually above
			// await this.postStateToWebview() // new AutoGen instance will post state when it's ready. having this here sent an empty messages array to webview leading to virtuoso having to reload the entire list
		}
	}
	...
	async updateCustomInstructions(instructions?: string) {
		...
		if (this.AutoGen) {
			this.AutoGen.customInstructions = instructions || undefined
		}
	}
	...
	async updateApiConfiguration(apiConfiguration: ApiConfiguration) {
		...
		await this.storeSecret("autogenApiKey", autogenApiKey)
		await this.storeSecret("sambanovaApiKey", sambanovaApiKey)
		if (this.AutoGen) {
			this.AutoGen.api = buildApiHandler(apiConfiguration)
		}
	}
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		const mcpServersDir = path.join(userDocumentsPath, "AutoGen", "MCP")
		try {
			await fs.mkdir(mcpServersDir, { recursive: true })
		} catch (error) {
			return "~/Documents/AutoGen/MCP" // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
		}
	...
	async handleAuthCallback(customToken: string, apiKey: string) {
		try {
			...
			await this.storeSecret("autogenApiKey", apiKey)
			...
			const autogenProvider: ApiProvider = "AutoGen"
			await this.updateGlobalState("apiProvider", autogenProvider)

			// Update API configuration with the new provider and API key
			const { apiConfiguration } = await this.getState()
			const updatedConfig = {
				...apiConfiguration,
				apiProvider: autogenProvider,
				autogenApiKey: apiKey,
			}

			if (this.AutoGen) {
				this.AutoGen.api = buildApiHandler(updatedConfig)
			}

			await this.postStateToWebview()
			vscode.window.showInformationMessage("Successfully logged in to AutoGen")
		} catch (error) {
			console.error("Failed to handle auth callback:", error)
			vscode.window.showErrorMessage("Failed to log in to AutoGen")
	...
	private async fetchMcpMarketplaceFromApi(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get("https://api.AutoGen.bot/v1/mcp/marketplace", {
				headers: {
	...
	private async downloadMcp(mcpId: string) {
		try {
			...
			const response = await axios.post<McpDownloadResponse>(
				"https://api.AutoGen.bot/v1/mcp/download",
				{ mcpId },
				{
...
- Use "${mcpDetails.mcpId}" as the server name in autogen_mcp_settings.json.
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
			...
			await this.initAutoGenWithTask(task)
			await this.postMessageToWebview({
				type: "action",
	...
	async handleOpenRouterCallback(code: string) {
		...
		await this.postStateToWebview()
		if (this.AutoGen) {
			this.AutoGen.api = buildApiHandler({
	...
	async showTaskWithId(id: string) {
		if (id !== this.AutoGen?.taskId) {
			// non-current task
			const { historyItem } = await this.getTaskWithId(id)
			await this.initAutoGenWithHistoryItem(historyItem) // clears existing task
		}
		await this.postMessageToWebview({
			type: "action",
			action: "chatButtonClicked",
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		return {
			...
			checkpointTrackerErrorMessage: this.AutoGen?.checkpointTrackerErrorMessage,
			autogenMessages: this.AutoGen?.autogenMessages || [],
	...
	async clearTask() {
		this.AutoGen?.abortTask()
		this.AutoGen = undefined // removes reference to it, so once promises end it will be garbage collected
	}

	// Caching mechanism to keep track of webview messages + API conversation history per provider instance

	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of AutoGen messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between AutoGenProvider instances since there could be multiple instances of the extension running at once. For example when we cached AutoGen messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
	- Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

	We need to use a unique identifier for each AutoGenProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
	...
	*/
	...
	async getState() {
		const [
			...
			this.getSecret("autogenApiKey") as Promise<string | undefined>,
		...
		const o3MiniReasoningEffort = vscode.workspace
			.getConfiguration("AutoGen.modelSettings.o3Mini")
