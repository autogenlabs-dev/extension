...
import "should"
import { AutoGenIgnoreController } from "./AutoGenIgnoreController"

describe("AutoGenIgnoreController", () => {
	let tempDir: string
	let controller: AutoGenIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .autogenignore file
		await fs.writeFile(
			path.join(tempDir, ".autogenignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new AutoGenIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .autogenignore file", async () => {
			const result = controller.validateAccess(".autogenignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .autogenignore", async () => {
			// Create a new .autogenignore with comments
			await fs.writeFile(
				path.join(tempDir, ".autogenignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .autogenignore gracefully", async () => {
			// Create a new controller in a directory without .autogenignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new AutoGenIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .autogenignore", async () => {
			await fs.writeFile(path.join(tempDir, ".autogenignore"), "")
