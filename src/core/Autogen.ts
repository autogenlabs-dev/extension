...
import {
	...
	AutoGenSay,
	AutoGenSayBrowserAction,
	AutoGenSayTool,
	COMPLETION_RESULT_CHANGES_FLAG,
} from "../shared/ExtensionMessage"
import { getApiMetrics } from "../shared/getApiMetrics"
import { HistoryItem } from "../shared/HistoryItem"
import { AutoGenAskResponse, AutoGenCheckpointRestore } from "../shared/WebviewMessage"
...
import { AutoGenProvider } from "./webview/AutogenProvider"
import { AutoGenIgnoreController, LOCK_TEXT_SYMBOL } from "./ignore/AutoGenIgnoreController"
import { AutoGenHandler } from "../api/providers/autogen"
...
export class AutoGen {
	...
	private autogenIgnoreController: AutoGenIgnoreController
	...
	constructor(
		...
		this.autogenIgnoreController = new AutoGenIgnoreController(cwd)
		this.autogenIgnoreController.initialize().catch((error) => {
			console.error("Failed to initialize AutoGenIgnoreController:", error)
		})
	...
	private async getSavedAutoGenMessages(): Promise<AutoGenMessage[]> {
	...
	private async addToAutoGenMessages(message: AutoGenMessage) {
		// these values allow us to reconstruct the conversation history at the time this AutoGen message was created
		// it's important that apiConversationHistory is initialized before we add AutoGen messages
		...
		this.autogenMessages.push(message)
		await this.saveAutoGenMessages()
	}

	private async overwriteAutoGenMessages(newMessages: AutoGenMessage[]) {
		this.autogenMessages = newMessages
		await this.saveAutoGenMessages()
	}

	private async saveAutoGenMessages() {
		try {
			const taskDir = await this.ensureTaskDirectoryExists()
			const filePath = path.join(taskDir, GlobalFileNames.uiMessages)
			await fs.writeFile(filePath, JSON.stringify(this.autogenMessages))
			// combined as they are in ChatView
			const apiMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(this.autogenMessages.slice(1))))
			const taskMessage = this.autogenMessages[0] // first message is always the task say
			const lastRelevantMessage =
				this.autogenMessages[
					findLastIndex(this.autogenMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))
				]
			let taskDirSize = 0
			try {
			...
			console.error("Failed to save AutoGen messages:", error)
		}
	}

	async restoreCheckpoint(messageTs: number, restoreType: AutoGenCheckpointRestore) {
		const messageIndex = this.autogenMessages.findIndex((m) => m.ts === messageTs)
		const message = this.autogenMessages[messageIndex]
		if (!message) {
			console.error("Message not found", this.autogenMessages)
			return
		}
		...
		if (!didWorkspaceRestoreFail) {
			switch (restoreType) {
				...
				case "taskAndWorkspace":
					...
					const deletedMessages = this.autogenMessages.slice(messageIndex + 1)
					const deletedApiReqsMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(deletedMessages)))

					const newAutoGenMessages = this.autogenMessages.slice(0, messageIndex + 1)
					await this.overwriteAutoGenMessages(newAutoGenMessages) // calls saveAutoGenMessages which saves historyItem
			...
			if (restoreType !== "task") {
				...
				const checkpointMessages = this.autogenMessages.filter((m) => m.say === "checkpoint_created")
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.autogenMessages.slice(0, messageIndex),
				...
				const firstCheckpointMessageCheckpointHash = this.autogenMessages.find(
	...
	async doesLatestTaskCompletionHaveNewChanges() {
		const messageIndex = findLastIndex(this.autogenMessages, (m) => m.say === "completion_result")
		const message = this.autogenMessages[messageIndex]
		if (!message) {
		...
		// Get last task completed
		const lastTaskCompletedMessage = findLast(this.autogenMessages.slice(0, messageIndex), (m) => m.say === "completion_result")
	...
	async ask(
		...
		if (partial !== undefined) {
			const lastMessage = this.autogenMessages.at(-1)
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.addToAutoGenMessages({
						ts: askTs,
						type: "ask",
						ask: type,
						text,
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: AutoGenAsk | AutoGenSay) {
		const lastMessage = this.autogenMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.autogenMessages.pop()
			await this.saveAutoGenMessages()
			await this.providerRef.deref()?.postStateToWebview()
		}
	}
	...
	private async startTask(task?: string, images?: string[]): Promise<void> {
		// conversationHistory (for API) and autogenMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the autogenMessages might not be empty, so we need to set it to [] when we create a new AutoGen client (otherwise webview would show stale messages from previous session)
		this.autogenMessages = []
		this.apiConversationHistory = []
	...
	private async resumeTaskFromHistory() {
		...
		const modifiedAutoGenMessages = await this.getSavedAutoGenMessages()

		// Remove any resume messages that may have been added before
		const lastRelevantMessageIndex = findLastIndex(
			modifiedAutoGenMessages,
			(m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"),
		)
		if (lastRelevantMessageIndex !== -1) {
			modifiedAutoGenMessages.splice(lastRelevantMessageIndex + 1)
		}
		...
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = modifiedAutoGenMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: AutoGenApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
			if (cost === undefined && cancelReason === undefined) {
				modifiedAutoGenMessages.splice(lastApiReqStartedIndex, 1)
			}
		}

		await this.overwriteAutoGenMessages(modifiedAutoGenMessages)
		this.autogenMessages = await this.getSavedAutoGenMessages()

		// Now present the AutoGen messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldnt be initialized when opening a old task, and it was because we were waiting for resume)
		// This is important in case the user deletes messages without resuming the task first
		this.apiConversationHistory = await this.getSavedApiConversationHistory()

		const lastAutoGenMessage = this.autogenMessages
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks
		// const lastAutoGenMessage = this.autogenMessages[lastAutoGenMessageIndex]
		// could be a completion result with a command
		// const secondLastAutoGenMessage = this.autogenMessages
		// 	.slice()
		// 	.reverse()
		// 	.find(
		// 		(m, index) =>
		// 			index !== lastAutoGenMessageIndex && !(m.ask === "resume_task" || m.ask === "resume_completed_task")
		// 	)
		// (lastAutoGenMessage?.ask === "command" && secondLastAutoGenMessage?.ask === "completion_result")

		let askType: AutoGenAsk
		if (lastAutoGenMessage?.ask === "completion_result") {
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with AutoGen messages
		...
		const agoText = (() => {
			const timestamp = lastAutoGenMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastAutoGenMessage?.ts && Date.now() - lastAutoGenMessage.ts < 30_000
		...
		newUserContent.push({
			...
			text:
				...
				(responseText
					? `\n\n${this.chatSettings?.mode === "chat" ? "New message to respond to with plan_mode_response tool (be sure to provide your response in the <response> parameter)" : "New instructions for task continuation"}:\n<user_message>\n${responseText}\n</user_message>`
					: this.chatSettings.mode === "chat"
	...
	private async initiateTaskLoop(userContent: UserContent, isNewTask: boolean): Promise<void> {
		...
		while (!this.abort) {
			const didEndLoop = await this.recursivelyMakeAutoGenRequests(nextUserContent, includeFileDetails, isNewTask)
			includeFileDetails = false // we only need file details the first time

			//  The way this agentic loop works is that AutoGen will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but AutoGen is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"AutoGen responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async abortTask() {
		...
		this.autogenIgnoreController.dispose()
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false) {
		...
		this.autogenMessages.forEach((message) => {
		...
		if (!isAttemptCompletionMessage) {
			...
			this.checkpointTracker?.commit().then(async (commitHash) => {
				const lastCheckpointMessage = findLast(this.autogenMessages, (m) => m.say === "checkpoint_created")
		...
		// for (let i = this.autogenMessages.length - 1; i >= 0; i--) {
		// 	const message = this.autogenMessages[i]
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const disableBrowserTool = vscode.workspace.getConfiguration("AutoGen").get<boolean>("disableBrowserTool") ?? false
		...
		const preferredLanguage = getLanguageKey(
			vscode.workspace.getConfiguration("AutoGen").get<LanguageDisplay>("preferredLanguage"),
		)
		const preferredLanguageInstructions =
			preferredLanguage && preferredLanguage !== DEFAULT_LANGUAGE_SETTINGS
				? `# Preferred Language\n\nSpeak in ${preferredLanguage}.`
				: ""
		const autogenRulesFilePath = path.resolve(cwd, GlobalFileNames.autogenRules)
		let autogenRulesFileInstructions: string | undefined
		if (await fileExistsAtPath(autogenRulesFilePath)) {
			if (await isDirectory(autogenRulesFilePath)) {
				try {
					// Read all files in the .autogenrules/ directory.
					const ruleFiles = await fs
						.readdir(autogenRulesFilePath, { withFileTypes: true, recursive: true })
						.then((files) => files.filter((file) => file.isFile()))
						.then((files) => files.map((file) => path.resolve(file.parentPath, file.name)))
					const ruleFileContent = await Promise.all(
						ruleFiles.map(async (file) => {
							const ruleFilePath = path.resolve(autogenRulesFilePath, file)
							const ruleFilePathRelative = path.relative(cwd, ruleFilePath)
							return `${ruleFilePathRelative}\n` + (await fs.readFile(ruleFilePath, "utf8")).trim()
						}),
					).then((contents) => contents.join("\n\n"))
					autogenRulesFileInstructions = `# .autogenrules/\n\nThe following is provided by a root-level .autogenrules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${ruleFileContent}`
				} catch {
					console.error(`Failed to read .autogenrules directory at ${autogenRulesFilePath}`)
				}
			} else {
				try {
					const ruleFileContent = (await fs.readFile(autogenRulesFilePath, "utf8")).trim()
					if (ruleFileContent) {
						autogenRulesFileInstructions = `# .autogenrules\n\nThe following is provided by a root-level .autogenrules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${ruleFileContent}`
					}
				} catch {
					console.error(`Failed to read .autogenrules file at ${autogenRulesFilePath}`)
				}
			}
		}

		const autogenIgnoreContent = this.autogenIgnoreController.autogenIgnoreContent
		let autogenIgnoreInstructions: string | undefined
		if (autogenIgnoreContent) {
			autogenIgnoreInstructions = `# .autogenignore\n\n(The following is provided by a root-level .autogenignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${autogenIgnoreContent}\n.autogenignore`
		}

		if (
			settingsCustomInstructions ||
			autogenRulesFileInstructions ||
			autogenIgnoreInstructions ||
		...
		if (previousApiReqIndex >= 0) {
			const previousRequest = this.autogenMessages[previousApiReqIndex]
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: AutoGenApiReqInfo = JSON.parse(previousRequest.text)
				...
				if (totalTokens >= maxAllowedSize) {
					...
					await this.saveAutoGenMessages() // saves task history item which we use to keep track of conversation history deleted range
		...
		try {
			...
			const isOpenRouter = this.api instanceof OpenRouterHandler || this.api instanceof AutoGenHandler
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				const askApproval = async (type: AutoGenAsk, partialMessage?: string) => {
				...
				switch (block.name) {
					...
					case "replace_in_file": {
						...
						try {
							...
							if (diff) {
								...
								// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but AutoGen throws error
							...
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to view directory ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to view source code definitions in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to search files in ${path.basename(absolutePath)}/`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
									const didApprove = await askApproval("tool", completeMessage)
									if (!didApprove) {
							...
							if (block.partial) {
								if (action === "launch") {
									...
									await this.say(
										...
										} satisfies AutoGenSayBrowserAction),
										undefined,
										block.partial,
									)
								}
								...
								if (action === "launch") {
									...
									if (this.shouldAutoApproveTool(block.name)) {
										...
										showNotificationForApprovalIfAutoApprovalEnabled(
											`AutoGen wants to use a browser and launch ${url}`,
										)
										this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
					...
					case "execute_command": {
						...
						try {
							if (block.partial) {
								...
								const ignoredFileAttemptedToAccess = this.autogenIgnoreController.validateCommand(command)
								if (ignoredFileAttemptedToAccess) {
									await this.say("autogenignore_error", ignoredFileAttemptedToAccess)
									pushToolResult(
										formatResponse.toolError(formatResponse.autogenIgnoreError(ignoredFileAttemptedToAccess)),
									)
								...
								if (!requiresApproval && this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to execute a command: ${command}`,
									)
					...
					case "use_mcp_tool": {
						...
						try {
							if (block.partial) {
								...
								} satisfies AutoGenAskUseMcpServer)
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`AutoGen tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
								...
								if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to use ${tool_name} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`AutoGen wants to access ${uri} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
									const didApprove = await askApproval("use_mcp_server", completeMessage)
									if (!didApprove) {
					...
					case "ask_followup_question": {
						...
						try {
							if (block.partial) {
								...
								if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
									showSystemNotification({
										subtitle: "AutoGen has a question...",
								...
								if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
									...
									const lastFollowupMessage = findLast(this.autogenMessages, (m) => m.ask === "followup")
					...
					case "plan_mode_response": {
						...
						const sharedMessage = {
							...
							options: parsePartialArrayString(removeClosingTag("options", optionsRaw)),
						} satisfies AutoGenPlanModeResponse
						try {
							if (block.partial) {
								...
								// if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
								// 	showSystemNotification({
								// 		subtitle: "AutoGen has a response...",
								...
								if (optionsRaw && text && parsePartialArrayString(optionsRaw).includes(text)) {
									// Valid option selected, don't show user message in UI
									// Update last followup message with selected option
									const lastPlanMessage = findLast(this.autogenMessages, (m) => m.ask === "plan_mode_response")
									if (lastPlanMessage) {
					...
					case "attempt_completion": {
						...
						if (command) {
							...
							// TODO: currently we don't handle if this command fails, it could be useful to let AutoGen know and retry
						...
						*/
						...
						const addNewChangesFlagToLastCompletionResultMessage = async () => {
							...
							const lastCompletionResultMessage = findLast(this.autogenMessages, (m) => m.say === "completion_result")
							if (
						...
						try {
							...
							if (block.partial) {
								if (command) {
									...
									// const secondLastMessage = this.autogenMessages.at(-2)
	...
	async recursivelyMakeAutoGenRequests(
		...
		if (this.consecutiveMistakeCount >= 3) {
			if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "AutoGen is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "AutoGen uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `AutoGen has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			await this.ask(
				"auto_approval_max_req_reached",
				`AutoGen has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
		...
		const previousApiReqIndex = findLastIndex(this.autogenMessages, (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.autogenMessages.filter((m) => m.say === "api_req_started").length === 0
		if (isFirstRequest) {
			...
			try {
				this.checkpointTracker = await pTimeout(
					CheckpointTracker.create(this.taskId, this.providerRef.deref()?.context.globalStorageUri.fsPath),
					{
						milliseconds: 15_000,
						message:
							"Checkpoints taking too long to initialize. Consider re-opening AutoGen in a project that uses git, or disabling checkpoints.",
					},
				)
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : "Unknown error"
				console.error("Failed to initialize checkpoint tracker:", errorMessage)
				this.checkpointTrackerErrorMessage = errorMessage // will be displayed right away since we saveAutoGenMessages next which posts state to webview
			}
		}
		...
		if (this.providerRef.deref()?.conversationTelemetryService.isOptedInToConversationTelemetry()) {
			...
			// Get the corresponding timestamp from autogenMessages
			// The last message in autogenMessages should be the one we just added

			const lastAutoGenMessage = this.autogenMessages[this.autogenMessages.length - 1]
			const ts = lastAutoGenMessage.ts
			...
			this.providerRef.deref()?.conversationTelemetryService.cleanupTask(this.taskId, this.autogenMessages)
		}

		// since we sent off a placeholder api_req_started message to update the webview while waiting to actually start the API request (to load potential details for example), we need to update the text of that message
		const lastApiReqIndex = findLastIndex(this.autogenMessages, (m) => m.say === "api_req_started")
		this.autogenMessages[lastApiReqIndex].text = JSON.stringify({
			request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
		} satisfies AutoGenApiReqInfo)
		await this.saveAutoGenMessages()
		await this.providerRef.deref()?.postStateToWebview()
		...
		try {
			...
			const updateApiReqMsg = (cancelReason?: AutoGenApiReqCancelReason, streamingFailedMessage?: string) => {
				this.autogenMessages[lastApiReqIndex].text = JSON.stringify({
					...JSON.parse(this.autogenMessages[lastApiReqIndex].text || "{}"),
			...
			const abortStream = async (cancelReason: AutoGenApiReqCancelReason, streamingFailedMessage?: string) => {
				if (this.diffViewProvider.isEditing) {
				...
				if (this.providerRef.deref()?.conversationTelemetryService.isOptedInToConversationTelemetry()) {
					...
					// Find the corresponding timestamp from autogenMessages
					// For assistant messages, we need to find the most recent "text" message
					const lastTextMessage = findLast(this.autogenMessages, (m) => m.say === "text")

					// Add the timestamp to the message object for telemetry
					if (!lastTextMessage) {
						console.error("No text message found in autogenMessages")
					} else {
						this.providerRef.deref()?.conversationTelemetryService.captureMessage(
							this.taskId,
							{
			...
			try {
				for await (const chunk of stream) {
					...
					if (this.abort) {
						...
						if (!this.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of AutoGen)
				...
				// abandoned happens when extension is no longer waiting for the AutoGen instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.abandoned) {
					...
					if (history) {
						await this.providerRef.deref()?.initAutoGenWithHistoryItem(history.historyItem)
						// await this.providerRef.deref()?.postStateToWebview()
					}
				}
			} finally {
			...
			// OpenRouter/AutoGen may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
			...
			if (assistantMessage.length > 0) {
				...
				if (this.providerRef.deref()?.conversationTelemetryService.isOptedInToConversationTelemetry()) {
					...
					if (lastAutoGenMessage) {
						this.providerRef.deref()?.conversationTelemetryService.captureMessage(
							this.taskId,
							{
								...lastMessage,
								ts: lastAutoGenMessage.ts,
							},
							{
								apiProvider: this.apiProvider,
								model: this.api.getModel().id,
				...
				const recDidEndLoop = await this.recursivelyMakeAutoGenRequests(this.userMessageContent)
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		// It could be useful for AutoGen to know if the user went from one or no file to another between messages, so we always include this context
		...
		// Filter paths through autogenIgnoreController
		const allowedVisibleFiles = this.autogenIgnoreController
		...
		// Filter paths through autogenIgnoreController
		const allowedOpenTabs = this.autogenIgnoreController
		...
		const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if AutoGen ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
		...
		*/
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(cwd, files, didHitLimit, this.autogenIgnoreController)
				details += result
			}
		}
		...
		if (this.chatSettings.mode === "chat") {
			details += "\nPLAN MODE"
			details +=
				"\nIn this mode you should focus on information gathering, asking questions, and architecting a solution. Once you have a chat, use the plan_mode_response tool to engage in a conversational back and forth with the user. Do not use the plan_mode_response tool until you've gathered all the information you need e.g. with read_file or ask_followup_question."
			details +=
				'\n(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the chat. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)'
		} else {
			details += "\nACT MODE"
		}
