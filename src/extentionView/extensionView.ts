...
import { showDesignContent as _showDesignContent, getDesignPrompt as _getDesignPrompt } from "./components/DesignPanel"
...
// Import ClineProvider to access its static methods and potentially types
import { ClineProvider } from "../core/webview/ClineProvider" // Updated import
...
class ExtensionView {
	...
	constructor(context: vscode.ExtensionContext) {
		this.panel = vscode.window.createWebviewPanel("autoGenBuilder", "AutoGen Code Builder", vscode.ViewColumn.One, {
			enableScripts: true,
					...
					switch (message.command) {
						case "createComponentFile":
							...
							try {
								...
								if (fs.existsSync(targetPath)) {
									...
									// --- START: New logic to trigger Cline task ---
									...
									if (clineProvider) {
										...
										const prompt =
											"Analyze the newly generated component with the following details:\\n" +
											"- Title: " +
											componentData.title +
											"\\n" +
											"- File path: " +
											...
											"\\n" +
											"- Dependencies: " +
											JSON.stringify(componentData.dependencies) +
											"\\n" +
											"- Language: " +
											...
											"\\n" +
											"- Difficulty: " +
											componentData.difficulty +
											"\\n" +
											"- Has animation: " +
											componentData.hasAnimation +
											"\\n" +
										...
										// Ensure the Cline panel is visible/focused
										// Use the correct command ID for the Cline view
										await vscode.commands.executeCommand("autogenlabs-code.SidebarProvider.focus") // Focus the view directly
										...
										console.log("Triggered new Cline task for:", targetPath)
									} else {
										console.warn("ClineProvider instance not found. Cannot trigger follow-up task.")
										vscode.window.showWarningMessage(
											"Component created, but couldn't automatically start analysis task. Please open the Cline chat manually.",
										)
									}
									// --- END: New logic ---
								} else {
								...
								vscode.window.showErrorMessage(
									`Failed to create component${errorMsgPath}: ${error.message}`,
								)
						...
						case "initializePrompt":
							{
								...
								if (clineProvider) {
									try {
										await vscode.commands.executeCommand("autogenlabs-code.SidebarProvider.focus")
										await new Promise((resolve) => setTimeout(resolve, 200))
										await clineProvider.initClineWithTask(message.prompt)
										console.log("Triggered Cline task with prompt from ExtensionView")
									} catch (error) {
										console.error("Failed to trigger Cline task:", error)
									}
								} else {
									console.warn("ClineProvider instance not found. Cannot trigger analysis task.")
									vscode.window.showWarningMessage(
										"Component created, but couldn't automatically start analysis task. Please open the Cline chat manually.",
									)
								}
						...
						case "setupReactTailwind": // New case to handle React + Tailwind setup
							...
							try {
								...
								this.panel.webview.postMessage({
									command: "error",
									message:
										"Failed to create React + Tailwind CSS project: " + (error as Error).message,
								})
							}
							break
						...
						case "setupNextjsTailwind": // Case to handle Next.js + Tailwind setup
							...
							try {
								...
								this.panel.webview.postMessage({
									command: "error",
									message:
										"Failed to create Next.js + Tailwind CSS project: " + (error as Error).message,
								})
							}
							break
						...
						case "setupNextjsBootstrap": // Case to handle Next.js + Bootstrap setup
							...
							try {
								...
								console.error(
									"Error setting up Next.js + Bootstrap project from extension host:",
									error,
								)
								this.panel.webview.postMessage({
									command: "error",
									message:
										"Failed to create Next.js + Bootstrap project: " + (error as Error).message,
								})
							}
							break
	...
	private getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri): string {
		const stylesUri = webview.asWebviewUri(
			vscode.Uri.file(path.join(extensionUri.fsPath, "src", "extentionView", "styles.css")),
		)
