...
describe("Extension Tests", function () {
	...
	it("should activate extension successfully", async () => {
		// Get the extension
		const extension = vscode.extensions.getExtension("autogenlabs")
	...
	it("should open sidebar view", async () => {
		...
		await vscode.commands.executeCommand("AutoGen.plusButtonClicked")
	...
	it("should handle basic commands", async () => {
		...
		await vscode.commands.executeCommand("AutoGen.historyButtonClicked")
		// Success if no error thrown
	})

	it("should handle advanced settings configuration", async () => {
		// Test browser session setting
		await vscode.workspace.getConfiguration().update("AutoGen.disableBrowserTool", true, true)
		const updatedConfig = vscode.workspace.getConfiguration("AutoGen")
		expect(updatedConfig.get("disableBrowserTool")).to.be.true

		// Reset settings
		await vscode.workspace.getConfiguration().update("AutoGen.disableBrowserTool", undefined, true)
	})
})
