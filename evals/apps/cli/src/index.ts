...
const run = async (toolbox: GluegunToolbox) => {
	...
	await execa({ cwd: exercisesPath })`git config user.name "Autogenlabs Code"`
...
if (!fs.existsSync(exercisesPath)) {
	console.error(
		`Exercises path does not exist. Please run "git clone https://github.com/cte/Autogenlabs-Code-Benchmark.git exercises".`,
	)
	process.exit(1)
}
