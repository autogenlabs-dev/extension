...
module.exports = {
	...
	moduleFileExtensions: ["ts", "tsx", "js", "jsx", "json", "node"],	transform: {
		"^.+\\.tsx?$": [
			"ts-jest",
			{
				tsconfig: process.env.USE_TEST_TSCONFIG ? "tsconfig.test.json" : {
					module: "CommonJS",
	...
	reporters: [["jest-simple-dot-reporter", {}]],	setupFiles: ["<rootDir>/src/__mocks__/jest.setup.ts"],
	setupFilesAfterEnv: [
		"<rootDir>/src/integrations/terminal/__tests__/setupTerminalTests.ts",
		"<rootDir>/src/__tests__/setupStripeTests.ts"
	],
}
